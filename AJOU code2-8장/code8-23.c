...
int g10[MAX_VERTEX][MAX_VERTEX] = {
	...
	{ INF, 1,   INF, 0,   4,   INF, INF},

	{ INF, 7,   INF, 4,   0,   INF, INF },

	{ INF, INF, 6,   INF, INF, 0,   9 },

	{ INF, INF, 2  , INF, INF, 9,   INF }
...
{
	int i, j, lap = 0;
	for (i = 0; i < MAX_VERTEX * MAX_VERTEX; ++i) //부모 찾기 초기화 (이번엔 간선 부모 찾기)
		par[i] = i;
	line line_arr[1000];
	for (i = 0; i < MAX_VERTEX; ++i) //간선 제작
		for (j = i + 1; j < MAX_VERTEX; ++j)
	...
	while (tree > 1)
	{
		...
		for (i = 0; i < lap; ++i)
		{
			int x = getPar(line_arr[i].s), y = getPar(line_arr[i].e);
			if (x == y)	continue; //사이클 간선 째낌
			if (least[x] == -1 || line_arr[least[x]].w > line_arr[i].w)	least[x] = i;
			if (least[y] == -1 || line_arr[least[y]].w > line_arr[i].w)	least[y] = i;
		}

		for (j = 0; j < MAX_VERTEX; ++j)
		{
			if (least[j] == -1)	continue;
			int x = getPar(line_arr[least[j]].s), y = getPar(line_arr[least[j]].e);
			if (x == y)	continue;
			printf("# of Trees:  %d:\n        (%d   %d)[%d]\n", tree, line_arr[least[j]].s, line_arr[least[j]].e, line_arr[least[j]].w);
			tree--;
			tie(x, y);
		}
	}
